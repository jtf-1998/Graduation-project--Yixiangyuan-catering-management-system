        ...
        getImgFile: function() {
            ...
            oFReader.onload = function(oFREvent) {
                ...
                qrcode.callback = function(data) {
                    //得到扫码的结果
                    if(data.search("DT") != -1){
                        $('.result-qrcode').append("桌号：大厅"+data.match(/\d+/) );
                    }
                    else if(data.search("YZ") != -1){
                        $('.result-qrcode').append("桌号：雅座"+data.match(/\d+/) );
                    }else if(data.search("BJ") != -1){
                            $('.result-qrcode').append("桌号：包间"+data.match(/\d+/) );
                    }
                    else {
                        $('.result-qrcode').append("请重新扫码");
                    }
                    $(".result-qrcode_display").append(data);

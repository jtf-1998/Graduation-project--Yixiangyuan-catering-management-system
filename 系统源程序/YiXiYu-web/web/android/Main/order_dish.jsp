...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var pRoomId = getQueryString("pRoomId");//拆分url得到”=”后面的参数
            var pRoomName=getQueryString("pRoomName");
            function Split(s){
                var ss;
                // 在每个字符T处进行分解。
                ss = s.split("：");
            ...
            pRoomName=Split(pRoomName)[1];
            pRoomName=trim(pRoomName,"g");
            // alert(pRoomName);
            $("#pRoomName").text(pRoomName);
            //菜单ID
            <c:set var="bill_id" value="${sessionScope.bill_id}" />
            var bill_id=${bill_id};
            // alert(bill_id);
             //获取当前日期时间
            var bill_data=CurentTime();
            function CurentTime()
            {
                var now = new Date();
            ...
            $(".add").click(function () {
                ...
                $.getJSON(
                    baseURL+"/android/CartServlet",
                    {
                        "bill_id":bill_id,
                        "bill_data":bill_data,
                        <%--"bill_customer":${sessionScope.phone},--%>
                        "bill_customer":"15612156587",
                        "pRoomName":pRoomName,
                        "vegetable_id":vegetableId.text(),
                        "vegetable_type":vegetableType.text(),
                        "vegetable_name":vegetableName.text(),
                        "vegetable_price":RemoveCh(vegetablePrice.text()),
                        "vegetable_number":1,
                        "vegetable_state":"未上",
                        "action":"doAdd"
                    },
            ...
            $(".minus").click(function () {
                ...
                var vegetablePrice = $(this).nextAll(".price");                  $.getJSON(
                baseURL+"/android/CartServlet",
                    {
                        "bill_id":bill_id,
                        "bill_data":bill_data,
            ...
            $("#btnselect").click(function () {
                window.location.href=encodeURI("${pageContext.request.contextPath}/android/CartServlet?action=doShowCart");
            })

document.writeln("<!DOCTYPE html>");
document.writeln("<html lang=\'en\'>");
document.writeln("<head>");
document.writeln("    <meta charset=\'UTF-8\'>");
document.writeln("    <title>底部导航栏</title>");
document.writeln("    <meta content=\'width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=0\' name=\'viewport\' />");
document.writeln("    <meta content=\'yes\' name=\'apple-mobile-web-app-capable\' />");
document.writeln("    <meta content=\'black\' name=\'apple-mobile-web-app-status-bar-style\' />");
document.writeln("    <meta content=\'telephone=no\' name=\'format-detection\' />");
document.writeln("    <!--<link href=\'../css/android_miner.css\' rel=\'stylesheet\' type=\'text/css\' />-->");
document.writeln("    <script src=\'../android/js/jquery.min.js\'></script>");
document.writeln("    <script type=\'text/javascript\'>");
document.writeln("        $(document).ready(function () {");
document.writeln("            var baseURL=\'${pageContext.request.contextPath}\';");
document.writeln("            $(\'#main\').click(function () {");
document.writeln("                window.location.href=\'../../android/BillOfFareServlet?action=doShow\';");
document.writeln("            })");
document.writeln("            $(\'#yuding\').click(function () {");
document.writeln("                window.location.href=\'../../android/Order/order.jsp\';");
document.writeln("            })");
document.writeln("            $(\'#zhangdan\').click(function () {");
document.writeln("                window.location.href=\'../../android/CustomerPayServlet?action=doShow\';");
document.writeln("            })");
document.writeln("            $(\'#gouwuche\').click(function () {");
document.writeln("                window.location.href=\'../../android/CartServlet?action=doShowCart\';");
document.writeln("            })");
document.writeln("            $(\'#wode\').click(function () {");
document.writeln("                window.location.href=\'../../android/MinnerServlet?action=doShow\';");
document.writeln("            })");
document.writeln("");
document.writeln("        });");
document.writeln("    </script>");
document.writeln("    <style type=\'text/css\'>");
document.writeln("        .footer_body{");
document.writeln("            height: 100%;");
document.writeln("            background: #efefef;");
document.writeln("        }");
document.writeln("        .aui-footer-fixed {");
document.writeln("            position: fixed;");
document.writeln("            bottom: 0;");
document.writeln("            left: 0;");
document.writeln("            z-index: 49;");
document.writeln("        }");
document.writeln("");
document.writeln("        .aui-footer {");
document.writeln("            width: 100%;");
document.writeln("            /*position: relative;*/");
document.writeln("            z-index: 100;");
document.writeln("            display: -webkit-box;");
document.writeln("            display: -webkit-flex;");
document.writeln("            display: -ms-flexbox;");
document.writeln("            display: flex;");
document.writeln("            -webkit-box-align: center;");
document.writeln("            -webkit-align-items: center;");
document.writeln("            -ms-flex-align: center;");
document.writeln("            align-items: center;");
document.writeln("            padding: 7px 5px 7px 5px;");
document.writeln("            background: #f9f9f9;");
document.writeln("        }");
document.writeln("");
document.writeln("        .aui-tabBar-item {");
document.writeln("            -webkit-box-flex: 1;");
document.writeln("            -webkit-flex: 1;");
document.writeln("            -ms-flex: 1;");
document.writeln("            flex: 1;");
document.writeln("            display: -webkit-box;");
document.writeln("            display: -webkit-flex;");
document.writeln("            display: -ms-flexbox;");
document.writeln("            display: flex;");
document.writeln("            -webkit-box-orient: vertical;");
document.writeln("            -webkit-box-direction: normal;");
document.writeln("            -webkit-flex-direction: column;");
document.writeln("            -ms-flex-direction: column;");
document.writeln("            flex-direction: column;");
document.writeln("            -webkit-box-pack: center;");
document.writeln("            -webkit-justify-content: center;");
document.writeln("            -ms-flex-pack: center;");
document.writeln("            justify-content: center;");
document.writeln("            -webkit-box-align: center;");
document.writeln("            -webkit-align-items: center;");
document.writeln("            -ms-flex-align: center;");
document.writeln("            align-items: center;");
document.writeln("            color: #979797;");
document.writeln("        }");
document.writeln("");
document.writeln("        * {");
document.writeln("            -webkit-box-sizing: border-box;");
document.writeln("            -moz-box-sizing: border-box;");
document.writeln("            box-sizing: border-box;");
document.writeln("        }");
document.writeln("");
document.writeln("        a {");
document.writeln("            text-decoration: none;");
document.writeln("            color: #000;");
document.writeln("        }");
document.writeln("");
document.writeln("        .img-loan {");
document.writeln("            background-image: url(\'data:image/png;base64,\');");
document.writeln("        }");
document.writeln("");
document.writeln("        .img-ions {");
document.writeln("            background-image: url(\'data:image/png;base64,\');");
document.writeln("        }");
document.writeln("");
document.writeln("        .img-credit {");
document.writeln("            background-image: url(\'data:image/png;base64,\');");
document.writeln("        }");
document.writeln("");
document.writeln("        .img-car {");
document.writeln("            background-image: url(\'data:image/png;base64,\');");
document.writeln("        }");
document.writeln("");
document.writeln("        .img-mine {");
document.writeln("            background-image: url(\'../images/minner2.png\');");
document.writeln("        }");
document.writeln("");
document.writeln("        .img {");
document.writeln("            width: 20px;");
document.writeln("            height: 20px;");
document.writeln("            display: block;");
document.writeln("            border: none;");
document.writeln("            float: left;");
document.writeln("            background-size: 20px;");
document.writeln("            background-repeat: no-repeat;");
document.writeln("        }");
document.writeln("");
document.writeln("    </style>");
document.writeln("");
document.writeln("</head>");
document.writeln("<body class=\'footer_body\'>·");
document.writeln("");
document.writeln("<footer class=\'aui-footer aui-footer-fixed\'>");
document.writeln("    <a target=\'footer\' class=\'aui-tabBar-item aui-tabBar-item-active\' >");
document.writeln("                <span class=\'aui-tabBar-item-img\'>");
document.writeln("                    <i class=\'img img-loan\'></i>");
document.writeln("                </span>");
document.writeln("        <span target=\'footer\' class=\'aui-tabBar-item aui-tabBar-item-active\' id=\'main\'>首页</span>");
document.writeln("    </a>");
document.writeln("    <a target=\'footer\' class=\'aui-tabBar-item \'>");
document.writeln("                <span class=\'aui-tabBar-item-img\'>");
document.writeln("                    <i class=\'img img-ions \'></i>");
document.writeln("                </span>");
document.writeln("        <span class=\'aui-tabBar-item-text\' id=\'yuding\'>预订</span>");
document.writeln("    </a>");
document.writeln("    <a target=\'footer\' class=\'aui-tabBar-item \'>");
document.writeln("                <span class=\'aui-tabBar-item-img\'>");
document.writeln("                    <i class=\'img img-credit\'></i>");
document.writeln("                </span>");
document.writeln("        <span class=\'aui-tabBar-item-text\' id=\'zhangdan\'>账单</span>");
document.writeln("    </a>");
document.writeln("    <a  target=\'footer\' class=\'aui-tabBar-item \'>");
document.writeln("                <span class=\'aui-tabBar-item-img\'>");
document.writeln("                    <i class=\'img img-car\'></i>");
document.writeln("                </span>");
document.writeln("        <span class=\'aui-tabBar-item-text\' id=\'gouwuche\'>购物车</span>");
document.writeln("    </a>");
document.writeln("");
document.writeln("    <a target=\'footer\' class=\'aui-tabBar-item \'>");
document.writeln("                <span class=\'aui-tabBar-item-img\'>");
document.writeln("                    <i class=\'img img-mine\'></i>");
document.writeln("                </span>");
document.writeln("        <span class=\'aui-tabBar-item-text\' id=\'wode\'>我的</span>");
document.writeln("    </a>");
document.writeln("</footer>");
document.writeln("</body>");
document.writeln("</html>");
document.writeln("");